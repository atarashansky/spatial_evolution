...
Round-9 nu-sweep additions (evolution_open_ns.py) -- MEASUREMENT ONLY, no
new RNG draws, trajectories bit-identical to evolution_open.py:
  * mode 0: exact first-passage frames of the t=0-cohort survivor count
    ns0 (number of distinct cohort lineages still present) to a descending
    list of thresholds ns0_thr, with the cohort cell-area at each crossing;
    ns0 and the cohort cell-area are maintained incrementally (a cohort
    lineage that hits size 0 can never be re-copied, so ns0 is monotone);
  * mode 0: second-largest lineage size mx2 and the number of heterolabel
    bonds b12 lying between the two largest lineages, at every sample
    (extra ts columns 10-13: mx2, b12, coh_live, largest_is_cohort);
  * mode 0: optional early stop T_stop_after (>0) once the cohort is
    extinct AND the frame index has passed T_stop_after.
...
def _run_open(fitness, label, nbr, n_nbr, sites, x_of, y_of, W, H, L_long,
              ...
              mode, nu, marker_sites, split,
              cnt, samp_frames, ts, hist, ck_idx, fp_thr, fp_frame,
              scal, scalf, ns0_thr, ns0_fp, ns0_coh, T_stop_after):
    ...
    scal (int64): 0 t_abs (frame relative to phase start; -1 none)
                  ...
                  8 ns0 at end (mode 0)   9 coh_live at end (mode 0)
    ...
    if mode == 0:
        ...
        ns0_live = m0
        coh_live = m0
        # thresholds already satisfied at t=0 (defensive; normally none)
        while ns0_i < n_ns0 and ns0_live <= ns0_thr[ns0_i]:
            ns0_fp[ns0_i] = 0
            ns0_coh[ns0_i] = coh_live
            ns0_i += 1
    elif mode == 1:
        for j in range(M):
            i = sites[j]
    ...
    for f in range(1, T_obs + 1):
        ...
        if samp_i < n_samp and f == samp_frames[samp_i]:
            if mode == 0:
                ...
                # b12 = heterolabel bonds whose two labels are the top-2 pair
                bdiff = np.int64(0)
                btot = np.int64(0)
                b12 = np.int64(0)
                for j in range(M):
                    ...
                    if xr <= L_long:
                        ...
                        if l2 >= 0:
                            ...
                            if l2 != li:
                                bdiff += 1
                                if ((li == id1 and l2 == id2)
                                        or (li == id2 and l2 == id1)):
                                    b12 += 1
                    ...
                    if l3 >= 0:
                        ...
                        if l3 != li:
                            bdiff += 1
                            if ((li == id1 and l3 == id2)
                                    or (li == id2 and l3 == id1)):
                                b12 += 1
                sfit = 0.0
                sfit2 = 0.0
                nl = np.int64(0)
                ...
                ts[samp_i, 10] = np.float64(mx2)
                ts[samp_i, 11] = np.float64(b12)
                ts[samp_i, 12] = np.float64(coh_live)
                ts[samp_i, 13] = 1.0 if (id1 >= 0 and id1 < m0) else 0.0
        ...
        # ---- cohort survivor first-passage (mode 0) + optional early stop
        if mode == 0:
            while ns0_i < n_ns0 and ns0_live <= ns0_thr[ns0_i]:
                ns0_fp[ns0_i] = f
                ns0_coh[ns0_i] = coh_live
                ns0_i += 1
            if T_stop_after > 0 and ns0_live == 0 and f >= T_stop_after:
                t_abs = f
                abs_type = 2
                break
...
def evolve_open(mode, L_long=192, L_short=64, T_burn=0, T_obs=1_000_000,
    ...
    if ns0_thr is None:
        ns0_thr = np.full(1, -1, np.int64)      # -1: never crossed
    ns0_thr = np.ascontiguousarray(ns0_thr, np.int64)
    ns0_fp = np.full(max(ns0_thr.size, 1), -1, np.int64)
    ns0_coh = np.full(max(ns0_thr.size, 1), -1, np.int64)

    scal = np.full(10, -1, np.int64)
    scalf = np.full(4, np.nan, np.float64)
    ...
    _run_open(fitness, label, nbr, np.int64(nbr.shape[1]), sites, x_of, y_of,
              ...
              cnt, samp_frames, ts, hist, ck_idx, fp_thr, fp_frame,
              scal, scalf, ns0_thr, ns0_fp, ns0_coh, np.int64(T_stop_after))
    ...
    out = {
        ...
        "ns0_thr": ns0_thr.copy(), "ns0_fp": ns0_fp.copy(),
        "ns0_coh": ns0_coh.copy(),
        "ns0_end": int(scal[8]), "coh_live_end": int(scal[9]),
