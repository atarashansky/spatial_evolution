"""Least-loaded-class census kernel.

Same dynamics as evolution_strip.py (dB Moran, Moore, alias-geometric deaths,
mutation of source AND daughter after every division), but the state carries an
integer deleterious hit count per cell (fitness = (1-sd)^count exactly) and the
sampler records mutation-load histograms at the interface and in the bulk,
per clone, plus the fitness gap g = ln fA - ln fB. Beneficials are off.
"""
import numpy as np
...
@njit
def _run_census(fitness, kcnt, label, nbr, n_nbr, sites, x_of, y_of, rowA,
                ...
                fA_t, fB_t, samp_frame, hbar_t, L_long, iface_w):
    np.random.seed(seed)
    M = sites.size
...
def census_strip(L_long=192, L_short=32, numgen=1_200_000, dt=2400.0,
    ...
    t_fix, winner, n_s = _run_census(
        ...
        nA_t, nB_t, fA_t, fB_t, samp_frame, hbar_t, np.int64(L_long), np.int64(iface_w))
    return dict(t_fix=int(t_fix), winner=int(winner),
                ...
                hbar=hbar_t[:n_s], p_die=p_die, M=int(M))
