...
`evolution_strip_mixed.py`; instrumented contact-census kernel `census2.py`, bit-for-bit identical to `strip_census.py` at equal seed). Ensembles were run on a 64-core driver (`ensemble_driver.py`); zero unexplained failures across the campaign. **Itemized total: 135,999 runs** across the 40 simulation ensembles itemized below (excluding exemplar renders; the S27 row is analysis-only and contributes no runs); the ensembles added or extended in revision are marked ‡, with the four round-8 rows (S27–S30) at the foot of the table. A further 9,160-run rule-switch ensemble (mixed dB/Bd 'pusher' clones), not part of the present analysis, is described separately.
...
| Fig. 4h, S21 | Front census functional form and contact structure (`census2.py` kernel) | 956 | 192×32 (14-rung θ ladder, 64/rung) + 192×128 (5-rung, 12/rung), s_d = 0.01 | to fixation | none (0 censored) | `contact_census_runs.parquet`, `n0_functional_form_and_contacts_summary.json` | exponential f₀ form fit; opposite-clone contact count 2.90; census closes ridge at s_d = 0.01 only |
| S22 | Fixed-s_d bracketing supplement for estimator robustness (`evolution_strip.py` kernel) | 2,100 | 192×32; 14 rungs × 150 runs at s_d = 0.0025, 0.005, 0.02 | to fixation | none (0 censored) | `estimator_supplement_runs.parquet`, `estimator_sensitivity_summary.json` | three ridge estimators agree per rung; legacy-anchor analysis, superseded by S25 |
| S23 | Click-onset effect-size sweep (round-6 test, against hypothesis; `sdr_kernel.py` instrumented kernel) | 5,360 | 192×32; s_d = 0.0025, 0.01, 0.04 ladders at 96/rung, densified 288/rung; 288 neutral; 464 pilot | to fixation | none (0 censored) | `onset_runs.parquet`, `onset_summary.json`, `onset_seed_dependence_check.json` | click-onset ∝ s_d^0.56 does not track ridge; landmark withdrawn; seed-cluster bootstrap slope 0.70 [0.41, 0.79] |
...
| S27 | Placement Monte Carlo and anchor leave-one-out (analysis of deposited runs; round-8) | 0 | analysis only: 10⁵ MC draws per tissue, 4 adversarial variants, 12 leave-one-out refits | — | — | `placement_mc_summary.json`, `placement_mc_draws.parquet`, `anchor_loo_table.csv` | colon P(fluid) = 0.9999; pooled bar shift ≤ 0.007 under any anchor leave-one-out |
| S28 | Ridge-law refit low-s_d extension rung (`evolution_strip.py` kernel; round-8) | 1,464 | 192×32; nine-rung θ ladder at s_d = 0.00125 (96/rung) plus fresh 600-run neutral anchor | to fixation | none (0 censored at the 4e+07-frame cap) | `low_sd_extension_runs.parquet`, `ridge_law_refit_summary.json`, `crossing_level_envelope.csv` | no crossing within U_d/s_d ≤ 1.6; left arm keeps rising; k = 1 leading order survives pooled re-anchoring (k = 1.15 ± 0.13) |
| S29 | Lattice-topology control: hexagonal and random-coordination stencils (`topo_strip.py` neighbour graphs on the production kernel; round-8) | 3,072 | 194×32; per stencil 600-run neutral anchor + nine-rung θ ladder (96/rung) at s_d = 0.01, plus 144 front-census runs | to fixation | none (0 censored at 2×10⁷) | `topology_control_runs.parquet`, `topology_control_summary.json` | ridges hex 0.33 [0.24, 0.38], random 0.34 [0.29, 0.44] — inside the Moore/vN envelope; ordering follows N_eff |
| S30 | Open-system ongoing-nucleation test (`evolution_open.py` kernel; round-8, against the closed-system extrapolation) | 13,100 | 192×64; lineage-founding ν = 5×10⁻⁴/birth (600 neutral + 400/loaded rung at θ = 0.2, 0.4, 0.8), single-cell marker (2,000/rung), disc marker r = 3 (600 neutral, 400/loaded), closed half-domain reference (600 neutral + 300/rung) | 3×10⁵ burn-in + 2×10⁶ observation (E1); to fixation (markers, reference) | none in the reference; cohort survivors alive at cap 0% | `open_system_runs.parquet`, `open_system_summary.json` | fluidization does NOT transfer: cohort half-life/neutral 0.98–0.99 vs closed T_fix/neutral 0.26 at θ = 0.8; nucleated competitor not advantaged (disc P(≥256)/neutral 0.59 at θ = 0.8) |
